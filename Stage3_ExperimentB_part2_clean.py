# In this script, we run the second section of the "Out Period" Experimental Setup.
# We apply the best configuration found from the random search in the first part of Experiment B to extract a list of anomaly clusters for the new timeframe.
# These are potential new media storms that need to be passed off to expert validation.
# and generate a list of potential new storms for expert validation. Finally, we generate tsne plots for the new time frame's anomalies.
...
## So, for example, these are the hyperparameters for 2007 based on the seed storms from 1996-2006:

INTERVAL_WIDTH = 0.25
CHANGEPOINT_PRIOR = 0.53
CHANGEPOINT_RANGE = 0.38
...
# --------------------------------------------------------------
# SETTING THE TIME FRAME FOR ANALYSIS
# --------------------------------------------------------------

## For example, in this run we seek to identify media storms in 2007
## We extract 10 years of data, 9 will include identified media storms and the 10th year will be the out-of-sample period
START = '1997-01-01'
END = '2007'
...
STORMS = STORMS[STORMS['start_date'] >= START]  # Only include storms that start after the given START date
STORMS = STORMS[STORMS['end_date'] < '2007-01-01']  # Exclude storms that extend into the 10th year (2007)
...
majority_clusters = majority_clusters[majority_clusters['LENGTH']>1]
majority_clusters = majority_clusters[majority_clusters['START']>=END] # only focus on the anomalies for the new timeframe
