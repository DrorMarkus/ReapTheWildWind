# In this script, we run the first section of the "Out Period" Experimental Setup.
# Essentially, we begin with a seed list of storms that we wish to use to calibrate the anomaly detection on a new, unanalyzed time frame.
# In this script, in a similar manner to Experiment A, we will use a random search to find the best hyperparameters for the anomaly detection.
...
# --------------------------------------------------------------
# ANOMALY DETECTION FUNCTIONS - identical to Experiment A
# --------------------------------------------------------------
...
for i, params in enumerate(tqdm(random_grid, desc='Random Search Iterations')):
    ...
    if (i + 1) % 100 == 0:
        ...
        simulation_df.to_csv(f'CalibrationResults_OutPeriod_RandomSearch_{i+1}.csv', index=False)
        print(f'Saved results after {i+1} iterations.')
        
# Save the final DataFrame after the loop
simulation_df = pd.DataFrame(simulation_dict)
simulation_df.to_csv('CalibrationResults_OutPeriod_RandomSearch_Final.csv', index=False)